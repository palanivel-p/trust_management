...
if(isset($_POST['doner_name']) && isset($_POST['mobile']) &&isset($_POST['amount'])) {
    ...
    if (mysqli_num_rows($resValidateCookie) > 0) {
          ...
          $sqlInsert = "INSERT INTO `transaction`(`pay_id`,`doner_name`,`doner_type`,`dob`,`address`,`mobile`,`pan`,`amount`,`pay_mode`,`added_by`,`branch_name`,`date`,`doner_id`,`mobile1`,`remark`,`transaction_id`,`type`) 
                                            VALUES ('','$doner_name','$doner_type','$dob','$address','$mobile','$pan','$amount','$pay_mode','$added_by','$branch_name','$date','$doneriD','$mobile1','$remark','$transaction_id','$type')";
            ...
            if(($_FILES['upload_image']['size']) <= $maxSize) {
                ...
                if (in_array($fileType, $allowTypes)) {
                    ...
                    if (!move_uploaded_file($_FILES["upload_image"]["tmp_name"], $targetFilePath)) {
                        ...
                        $info = urlencode("Added Transaction" );
